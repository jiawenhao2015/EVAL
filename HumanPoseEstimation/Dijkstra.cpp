...
Dijkstra::Dijkstra(int Num, int CenterX, int CenterY, int scale, int step)
{
	...
	Mat tempp(HEIGHT, WIDTH, CV_32FC3);
	tempp.copyTo(distanceMap);

	for (int i = 0; i < HEIGHT; i++)				////初始化map
	{
...
{
	Mat temp(HEIGHT, WIDTH, CV_32FC3);/////
	temp.copyTo(distanceMap);

	for (int i = 0; i<WIDTH; i++)				////初始化map
	{
		for (int j = 0; j<WIDTH; j++)
		{
			map.at<Vec3b>(i, j)[0] = 255;
...
{
	Mat tempmap(HEIGHT, WIDTH, CV_32FC3);/////
	map.copyTo(tempmap);
...
void Dijkstra::getIsolatedSet(Mat &map, bool flag)
{
	...
	for (int i = 0; i < WIDTH; i = i + downSampling)
	{

		for (int j = 0; j < HEIGHT; j = j + downSampling)
		{
			if (map.at<Vec3b>(i, j)[0] == 0)///r g b中的b分量为0 背景颜色为白色 三分量均为255 //找到第一个区域
			{
				cout << "found~~" << j << " " << i << endl;
